...
namespace Customer
{
    public class CustomerValidator
    {
        ...
        private static readonly Regex _emailRegex = new Regex(@"^([\w\.\-]+)@([\w\-]+)((\.(\w){2,3})+)$");

        public static List<string> Validate(Customer customer)
        {
            var errors = new List<string>();

            if (customer.FirstName != null && customer.FirstName.Length > FirstNameMaxLength)
            {
                errors.Add("Invalid customer property: FirstName");
            }
            if (string.IsNullOrEmpty(customer.LastName) || customer.LastName.Length > LastNameMaxLength)
            {
